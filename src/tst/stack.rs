...
use mru::lockfree::{ Stack, StackConsumer, StackProducer };
...
fn test_concurrent_produce_consume(
  ...
  let stack: Stack<usize> = Stack::new(128);
  let arc_stack = Arc::new(stack);
  let stack_producer = StackProducer::new(&arc_stack);
  let stack_consumer = StackConsumer::new(&arc_stack);
